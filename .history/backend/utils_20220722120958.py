...
def get_top_15(tweets_data_frame):
    ...
    for i in range(tweets_data_frame.shape[0]):
      ...
      print(tweets_data_frame.iloc[i]["clean_text"], file=sys.stderr)
      print(tweets_data_frame.iloc[i]["text"], file=sys.stderr)
      print('#'*30, file=sys.stderr)
      for word in list_words:
        if word not in word_dict.keys():
          word_dict[word] = 0
        word_dict[word] += 1
    print(word_dict, file=sys.stderr)
    print(tweets_data_frame.shape, file=sys.stderr)
...
def stock_price_graph(stock_name):
    ...
    print(price_history['Open'].keys(), time_series)
    lst = []
    for key, val in price_history['Open'].items():
        lst.append([key, val])
    return lst
