...
def stock_price_graph(stock_name):
    ...
    # dt_list = [dt for dt in price_history.index]
    print(price_history.index,file=sys.stderr)
    val = list(map(lambda x: [str(x[0]).split(' ')[0].replace('-',' '),int(x[1])],filter(lambda arr: not math.isnan(arr[1]),zip(price_history.index,price_history['Open']))))
