...
def stock_price_graph(stock_name):
    ...
    dt_list = [int(pendulum.parse(str(dt)).float_timestamp) for dt in list(price_history.index)]
    print(dt_list, file=sys.stderr)
    print(type(price_history['Open'].items()), file=sys.stderr)
    print(zip(dt_list,price_history['Open'].values()), file=sys.stderr)
    lst = []
    for key, val in zip(dt_list,price_history['Open'].values()):#price_history['Open'].items():
        lst.append([key, val])
    return lst
