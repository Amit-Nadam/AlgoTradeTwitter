...
def stock_price_graph(stock_name):
    price_history = yf.Ticker(stock_name).history(period='1y', # valid periods: 1d,5d,1mo,3mo,6mo,1y,2y,5y,10y,ytd,max
                                    ...
                                    actions=False)
    # time_series = pd.to_datetime(price_history['Open']).astype('int')
    dt_list = [dt for dt in pd.to_datetime(price_history.index).astype('int')]
    val = list(filter(lambda arr: not math.isnan(arr[1]),zip(dt_list,price_history['Open'])))
    print( list(zip(dt_list,price_history['Open'])), file=sys.stderr)
    lst = []
    for key, val in zip(dt_list,prices):#price_history['Open'].items():
      print(type(key),type(val), file=sys.stderr)
